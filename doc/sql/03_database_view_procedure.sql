...
CREATE PROCEDURE voir_recette (INT)
LANGUAGE SQL
AS $$
    ...
    WHERE recette_id = $1
	UNION
	SELECT designation_technique FROM recette WHERE recette.id = $1;
$$;
...
CREATE PROCEDURE entete_commande(INT) 
LANGUAGE SQL
AS $$
	SELECT 
		TO_CHAR(commande.date , 'HH12:MI:SS') AS Date, 
		CONCAT(users.prenom, ' ', users.nom) AS Client, 
		...
		CONCAT(paiement_type.designation, CASE WHEN paiement THEN ' OK' ELSE ' ...' END) AS Paiement
	FROM commande
	JOIN users ON users.id = commande.client_id
	JOIN status_commande ON status_commande.id = commande.status_id
